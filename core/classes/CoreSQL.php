...
class coreSQL extends coreObj{
    ...
    public function __construct($name=null, $options=array()){
        ...
        if($this->dbSettings['driver'] == 'pdo' && !class_exists('CSCMS\Core\Drivers\PDOMysql', false)){
            trigger_error('Error: You have selected to use PDO, the interface for this Driver dosen\'t exist.', E_USER_ERROR);
        }

        if($this->dbSettings['driver'] == 'mysqli' && ( !class_exists('CSCMS\Core\Drivers\Mysqli', false) || !class_exists('mysqli', false) )){
            trigger_error('Error: You have selected to use MySQLi, the interface for this Driver dosen\'t exist.', E_USER_ERROR);
        }

        if($this->dbSettings['driver'] == 'mysql' && ( !class_exists('CSCMS\Core\Drivers\Mysql', false) || !function_exists('mysql_connect') )){
...
**/
    ...
    public function queryBuilder(){
        ...
        $classname = 'CSCMS\\Core\\Drivers\\'.$driver.'QueryBuilder';
